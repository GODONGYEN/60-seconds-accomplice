...
플레이어는 제한된 시간 동안 보안 시설에 침입한다. 시간이 끝나거나 플레이어가 사망하면 회차가 초기화되지만, 이전 회차의 플레이어 행동은 Ghost로 남아 다음 회차에서 그대로 재생된다.
...
prototype에서 사망 시스템이 아직 없으면 시간 종료와 수동 restart만 사용한다.
...
- 적에게 보이는지 여부는 스테이지 또는 업그레이드 규칙으로 결정한다.
...
> 과거의 내가 압력판을 누르고 현재의 내가 문을 통과하는 순간이 실제로 재미있는가?
