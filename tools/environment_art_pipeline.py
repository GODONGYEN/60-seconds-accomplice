...
def validate() -> None:
    ...
    expected_outputs = (PROCESSED_ATLAS, RUNTIME_ATLAS, PREVIEW_PATH, PALETTE_PREVIEW_PATH, TILESET_PATH)
...
def fingerprint() -> None:
    paths: Iterable[Path] = (
        ...
        PALETTE_PREVIEW_PATH, RUNTIME_ATLAS, TILESET_PATH,
    )
    for path in paths:
        print(f"{path.relative_to(ROOT)} {_sha256(path)}")
